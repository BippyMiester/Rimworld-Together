...
namespace GameServer
{
    public static class CommandStorage
    {
        ...
        #region RconCommands
            private static readonly RconCommand rconHelpCommand = new RconCommand("help", 0,
            "Shows a list of all available commands to use",
                RconHelpCommandAction);

            private static readonly RconCommand rconBackupCommand = new RconCommand("backup", 0,
                "Backup the server.",
                RconBackupCommandAction);

            private static readonly RconCommand rconBackupUserCommand = new RconCommand("backupuser", 1,
                "Backup the data of a specific user",
                RconBackupUserCommandAction);

            private static readonly RconCommand rconListCommand = new RconCommand("list", 0,
                "Shows all connected players",
                RconListCommandAction);

            private static readonly RconCommand rconOpCommand = new RconCommand("op", 1,
                "Gives admin privileges to the selected player",
                RconOpCommandAction);

            private static readonly RconCommand rconDeopCommand = new RconCommand("deop", 1,
                "Removes admin privileges from the selected player",
                RconDeopCommandAction);

            private static readonly RconCommand rconKickCommand = new RconCommand("kick", 1,
                "Kicks the selected player from the server",
                RconKickCommandAction);

            private static readonly RconCommand rconBanCommand = new RconCommand("ban", 1,
                "Bans the selected player from the server",
                RconBanCommandAction);

            private static readonly RconCommand rconPardonCommand = new RconCommand("pardon", 1,
                "Pardons the selected player from the server",
                RconPardonCommandAction);

            private static readonly RconCommand rconDeepListCommand = new RconCommand("deeplist", 0,
                "Shows a list of all server players",
                RconDeepListCommandAction);

            private static readonly RconCommand rconBanListCommand = new RconCommand("banlist", 0,
                "Shows a list of all banned server players",
                RconBanListCommandAction);

            private static readonly RconCommand rconReloadCommand = new RconCommand("reload", 0,
                "Reloads all server resources",
                RconReloadCommandAction);

            private static readonly RconCommand rconModListCommand = new RconCommand("modlist", 0,
                "Shows all currently loaded mods",
                RconModListCommandAction);

            private static readonly RconCommand rconDoSiteRewards = new RconCommand("dositerewards", 0,
                "Forces site rewards to run",
                RconDoSiteRewardsCommandAction);

            private static readonly RconCommand rconEventCommand = new RconCommand("event", 2,
                "Sends a command to the selecter players",
                RconEventCommandAction);

            private static readonly RconCommand rconEventAllCommand = new RconCommand("eventall", 1,
                "Sends a command to all connected players",
                RconEventAllCommandAction);

            private static readonly RconCommand rconEventListCommand = new RconCommand("eventlist", 0,
                "Shows a list of all available events to use",
                RconEventListCommandAction);

            private static readonly RconCommand rconBroadcastCommand = new RconCommand("broadcast", -1,
                "Broadcast a message to all connected players",
                RconBroadcastCommandAction);

            private static readonly RconCommand rconServerMessageCommand = new RconCommand("chat", -1,
                "Send a message in chat from the Server",
                RconServerMessageCommandAction);

            private static readonly RconCommand rconWhitelistCommand = new RconCommand("whitelist", 0,
                "Shows all whitelisted players",
                RconWhitelistCommandAction);

            private static readonly RconCommand rconWhitelistAddCommand = new RconCommand("whitelistadd", 1,
                "Adds a player to the whitelist",
                RconWhitelistAddCommandAction);

            private static readonly RconCommand rconWhitelistRemoveCommand = new RconCommand("whitelistremove", 1,
                "Removes a player from the whitelist",
                RconWhitelistRemoveCommandAction);

            private static readonly RconCommand rconWhitelistToggleCommand = new RconCommand("togglewhitelist", 0,
                "Toggles the whitelist ON or OFF",
                RconWhitelistToggleCommandAction);

            private static readonly RconCommand rconForceSaveCommand = new RconCommand("forcesave", 1,
                "Forces a player to sync their save",
                RconForceSaveCommandAction);

            private static readonly RconCommand rconResetPlayerCommand = new RconCommand("resetplayer", 1,
                "Resets a player profile from the server",
                RconResetPlayerCommandAction);

            private static readonly RconCommand rconToggleDifficultyCommand = new RconCommand("toggledifficulty", 0,
                "Enables custom difficulty in the server",
                RconToggleDifficultyCommandAction);

            private static readonly RconCommand rconToggleCustomScenariosCommand = new RconCommand("togglecustomscenarios", 0,
                "enables/disables custom scenarios on the server",
                RconToggleCustomScenariosCommandAction);

            private static readonly RconCommand rconToggleDiscordPresenceCommand = new RconCommand("togglediscordpresence", 0,
                "enables/disables Discord pressence on the server",
                RconToggleDiscordPressenceCommandAction);

            private static readonly RconCommand rconToggleUPnPCommand = new RconCommand("toggleupnp", 0,
                "enables/disables UPnP port mapping (auto-portforwarding)",
                RconToggleUPnPCommandAction);

            private static readonly RconCommand rconPortforwardCommand = new RconCommand("portforward", 0,
                "will use UPnP to portforward the server",
                RconPortForwardCommandAction);

            private static readonly RconCommand rconToggleVerboseLogsCommand = new RconCommand("toggleverboselogs", 0,
                "toggles verbose logs to be true or false",
                RconToggleVerboseLogsCommandAction);

            private static readonly RconCommand rconToggleExtremeVerboseLogsCommand = new RconCommand("toggleextremeverboselogs", 0,
                "toggles extreme verbose logs to be true or false",
                RconToggleExtremeVerboseLogsCommandAction);

            private static readonly RconCommand rconToggleSyncLocalSaveCommand = new RconCommand("togglesynclocalsave", 0,
                "toggles allowing local saves to sync with server to be true or false",
                RconToggleSyncLocalSaveCommandAction);

            private static readonly RconCommand rconSetGameSpeedCommand = new RconCommand("setgamespeed", 1,
                "Changes the enforced game speed for all players",
                RconSetGameSpeedCommandAction);

            private static readonly RconCommand rconResetWorldCommand = new RconCommand("resetworld", 0,
                "Resets all the world related data and stores a backup of it",
                RconResetWorldCommandAction);

            private static readonly RconCommand rconQuitCommand = new RconCommand("quit", 0,
                "Saves all player data and then closes the server",
                RconQuitCommandAction);

            private static readonly RconCommand rconForceQuitCommand = new RconCommand("forcequit", 0,
                "Closes the server without saving player data",
                RconForceQuitCommandAction);

            private static readonly RconCommand rconClearCommand = new RconCommand("clear", 0,
                "Clears the console output",
                RconClearCommandAction);

            private static readonly RconCommand rconShowModManagerCommand = new RconCommand("showmodmanager", 1,
                "Allows a player to change mod configuration for the server",
                RconShowModManagerCommandAction);

            private static readonly RconCommand rconUpdateCommand = new RconCommand("update", 0,
                "Updates your server to the newest version. Do not use if you aren't told directly to do so, as it can very well BREAK things",
                RconUpdateCommandAction);

            public static List<RconCommand> rconCommands = new List<RconCommand>
            {
                rconHelpCommand,
                rconBackupCommand,
                rconBackupUserCommand,
                rconListCommand,
                rconOpCommand,
                rconDeopCommand,
                rconKickCommand,
                ...
                rconEventCommand,
                rconEventAllCommand,
                rconEventListCommand,
                rconBroadcastCommand,
                rconServerMessageCommand,
                rconWhitelistCommand,
                rconWhitelistAddCommand,
                rconWhitelistRemoveCommand,
                rconWhitelistToggleCommand,
                rconForceSaveCommand,
                rconResetPlayerCommand,
                rconToggleDifficultyCommand,
                rconToggleCustomScenariosCommand,
                rconToggleDiscordPresenceCommand,
                rconToggleUPnPCommand,
                rconPortforwardCommand,
                rconToggleVerboseLogsCommand,
                rconToggleExtremeVerboseLogsCommand,
                rconToggleSyncLocalSaveCommand,
                rconSetGameSpeedCommand,
                rconResetWorldCommand,
                rconQuitCommand,
                rconForceQuitCommand,
                rconClearCommand,
                rconShowModManagerCommand,
                rconUpdateCommand
            };
            
            private static void RconHelpCommandAction(IWebSocketConnection socket)
            {
                // Start building the output message
                var messageBuilder = new System.Text.StringBuilder();

                // Add title and header
                messageBuilder.AppendLine($"List of available commands: [{rconCommands.Count()}]");
                messageBuilder.AppendLine("----------------------------------------");

                // Append each command's prefix and description
                foreach (RconCommand command in rconCommands.ToList().OrderBy(fetch => fetch.prefix))
                {
                    messageBuilder.AppendLine($"{command.prefix} - {command.description}");
                }

                // Add footer
                messageBuilder.AppendLine("----------------------------------------");

                // Send the accumulated message to the client
                socket.Send(messageBuilder.ToString());
            }
            
            private static void RconBackupCommandAction(IWebSocketConnection socket) 
            {
                BackupManager.BackupServer();
            }

            private static void RconBackupUserCommandAction(IWebSocketConnection socket) 
            {
                UserFile userFile = UserManagerHelper.GetUserFileFromName(CommandManager.commandParameters[0]);
            ...
            private static void RconListCommandAction(IWebSocketConnection socket)
            {
                Logger.Title($"Connected players: [{NetworkHelper.GetConnectedClientsSafe().Count()}]");
                Logger.Title("----------------------------------------");
                foreach (ServerClient client in NetworkHelper.GetConnectedClientsSafe())
            ...
            private static void RconDeepListCommandAction(IWebSocketConnection socket)
            {
                UserFile[] userFiles = UserManagerHelper.GetAllUserFiles();
            ...
            private static void RconOpCommandAction(IWebSocketConnection socket)
            {
                ServerClient toFind = Network.connectedClients.ToList().Find(x => x.userFile.Username == CommandManager.commandParameters[0]);
                if (toFind == null) Logger.Warning($"User '{CommandManager.commandParameters[0]}' was not found");
            ...
            private static void RconDeopCommandAction(IWebSocketConnection socket)
            {
                ServerClient toFind = Network.connectedClients.ToList().Find(x => x.userFile.Username == CommandManager.commandParameters[0]);
                if (toFind == null) Logger.Warning($"User '{CommandManager.commandParameters[0]}' was not found");
            ...
            private static void RconKickCommandAction(IWebSocketConnection socket)
            {
                ServerClient toFind = Network.connectedClients.ToList().Find(x => x.userFile.Username == CommandManager.commandParameters[0]);
                if (toFind == null) Logger.Warning($"User '{CommandManager.commandParameters[0]}' was not found");
            ...
            private static void RconBanListCommandAction(IWebSocketConnection socket)
            {
                List<UserFile> userFiles = UserManagerHelper.GetAllUserFiles().ToList().FindAll(x => x.IsBanned);
            ...
            private static void RconBanCommandAction(IWebSocketConnection socket) { UserManager.BanPlayerFromName(CommandManager.commandParameters[0]); }

            private static void RconPardonCommandAction(IWebSocketConnection socket) { UserManager.PardonPlayerFromName(CommandManager.commandParameters[0]); }

            private static void RconReloadCommandAction(IWebSocketConnection socket) { Main_.LoadResources(); }
            
            private static void RconModListCommandAction(IWebSocketConnection socket)
            {
                Logger.Title($"Required Mods: [{Master.modConfig.RequiredMods.Length}]");
                Logger.Title("----------------------------------------");
                foreach (string str in Master.modConfig.RequiredMods) Logger.Warning($"{str}");
            ...
            private static void RconSetGameSpeedCommandAction(IWebSocketConnection socket)
            {
                int desiredSpeed = int.Parse(CommandManager.commandParameters[0]);
                if (desiredSpeed < 0 || desiredSpeed > 4) Logger.Error("Tried to set invalid game speed, specify 0-4");
                else
            ...
            private static void RconDoSiteRewardsCommandAction(IWebSocketConnection socket)
            {
                Logger.Title($"Forced site rewards");
                SiteManager.SiteRewardTick();
            }

            private static void RconEventCommandAction(IWebSocketConnection socket)
            {
                ServerClient client = Network.connectedClients.ToList().Find(x => x.userFile.Username == CommandManager.commandParameters[0]);
                if (client == null) Logger.Warning($"User '{CommandManager.commandParameters[0]}' was not found");
                else
            ...
            private static void RconEventAllCommandAction(IWebSocketConnection socket)
            {
                EventFile toFind = EventManagerHelper.loadedEvents.FirstOrDefault(fetch => fetch.DefName == CommandManager.commandParameters[0]);
                if (toFind == null) Logger.Warning($"Event '{CommandManager.commandParameters[0]}' was not found");
                else
            ...
            private static void RconEventListCommandAction(IWebSocketConnection socket)
            {
                Logger.Title($"Available events: [{EventManagerHelper.loadedEvents.Length}]");
                Logger.Title("----------------------------------------");
                foreach (EventFile eventFile in EventManagerHelper.loadedEvents) Logger.Warning($"{eventFile.DefName}");
            ...
            private static void RconBroadcastCommandAction(IWebSocketConnection socket)
            {
                string fullText = "";
                foreach (string str in CommandManager.commandParameters) fullText += $"{str} ";
                fullText = fullText.Remove(fullText.Length - 1, 1);
            ...
            private static void RconServerMessageCommandAction(IWebSocketConnection socket)
            {
                string fullText = "";
                foreach (string str in CommandManager.commandParameters)
                {
            ...
            private static void RconWhitelistCommandAction(IWebSocketConnection socket)
            {
                Logger.Title($"Whitelisted usernames: [{Master.whitelist.WhitelistedUsers.Count()}]");
                Logger.Title("----------------------------------------");
                foreach (string str in Master.whitelist.WhitelistedUsers) Logger.Warning($"{str}");
            ...
            private static void RconWhitelistAddCommandAction(IWebSocketConnection socket)
            {
                UserFile userFile = UserManagerHelper.GetUserFileFromName(CommandManager.commandParameters[0]);
                if (userFile == null) Logger.Warning($"User '{CommandManager.commandParameters[0]}' was not found");
            ...
            private static void RconWhitelistRemoveCommandAction(IWebSocketConnection socket)
            {
                UserFile userFile = UserManagerHelper.GetUserFileFromName(CommandManager.commandParameters[0]);
                if (userFile == null) Logger.Warning($"User '{CommandManager.commandParameters[0]}' was not found");
            ...
            private static void RconWhitelistToggleCommandAction(IWebSocketConnection socket) { WhitelistManager.ToggleWhitelist(); }

            private static void RconForceSaveCommandAction(IWebSocketConnection socket)
            {
                ServerClient toFind = Network.connectedClients.ToList().Find(x => x.userFile.Username == CommandManager.commandParameters[0]);
                if (toFind == null) Logger.Warning($"User '{CommandManager.commandParameters[0]}' was not found");
            ...
            private static void RconResetPlayerCommandAction(IWebSocketConnection socket)
            {
                UserFile userFile = UserManagerHelper.GetUserFileFromName(CommandManager.commandParameters[0]);
                ServerClient toFind = NetworkHelper.GetConnectedClientFromUsername(userFile.Username);
            ...
            private static void RconToggleDifficultyCommandAction(IWebSocketConnection socket)
            {
                Master.difficultyValues.UseCustomDifficulty = !Master.difficultyValues.UseCustomDifficulty;
                Logger.Warning($"Custom difficulty is now {(Master.difficultyValues.UseCustomDifficulty ? ("Enabled") : ("Disabled"))}");
                Main_.SaveValueFile(ServerFileMode.Difficulty);
            ...
            private static void RconToggleCustomScenariosCommandAction(IWebSocketConnection socket)
            {
                Master.actionValues.EnableCustomScenarios = !Master.actionValues.EnableCustomScenarios;
                Logger.Warning($"Custom scenarios are now {(Master.actionValues.EnableCustomScenarios ? ("Enabled") : ("Disabled"))}");
                Main_.SaveValueFile(ServerFileMode.Configs);
            ...
            private static void RconToggleDiscordPressenceCommandAction(IWebSocketConnection socket)
            {
                Master.discordConfig.Enabled = !Master.discordConfig.Enabled;
                Logger.Warning($"Discord pressence is now {(Master.discordConfig.Enabled ? ("Enabled") : ("Disabled"))}");
                Logger.Warning("Please restart the server to start the service");
            ...
            private static void RconToggleUPnPCommandAction(IWebSocketConnection socket)
            {
                Master.serverConfig.UseUPnP = !Master.serverConfig.UseUPnP;
                Logger.Warning($"UPnP port mapping is now {(Master.serverConfig.UseUPnP ? ("Enabled") : ("Disabled"))}");
                Main_.SaveValueFile(ServerFileMode.Configs);
            ...
            private static void RconPortForwardCommandAction(IWebSocketConnection socket)
            {
                if (!Master.serverConfig.UseUPnP)
                {
                    Logger.Error("Cannot portforward because UPnP is disabled on the server. You can use the command 'toggleupnp' to enable it.");
            ...
            private static void RconToggleVerboseLogsCommandAction(IWebSocketConnection socket)
            {
                Master.serverConfig.VerboseLogs = !Master.serverConfig.VerboseLogs;
                Logger.Warning($"Verbose Logs set to {Master.serverConfig.VerboseLogs}");
                Main_.SaveValueFile(ServerFileMode.Configs);
            ...
            private static void RconToggleExtremeVerboseLogsCommandAction(IWebSocketConnection socket)
            {
                Master.serverConfig.ExtremeVerboseLogs = !Master.serverConfig.ExtremeVerboseLogs;
                Logger.Warning($"Extreme verbose Logs set to {Master.serverConfig.ExtremeVerboseLogs}");
                Main_.SaveValueFile(ServerFileMode.Configs);
            ...
            private static void RconToggleSyncLocalSaveCommandAction(IWebSocketConnection socket)
            {
                Master.serverConfig.SyncLocalSave = !Master.serverConfig.SyncLocalSave;
                Logger.Warning($"Sync Local Save set to {Master.serverConfig.SyncLocalSave}");
                Main_.SaveValueFile(ServerFileMode.Configs);
            ...
            private static void RconResetWorldCommandAction(IWebSocketConnection socket)
            {
                //Make sure the user wants to reset the world
                Logger.Warning("Are you sure you want to reset the world?");
                Logger.Warning("Please type 'YES' or 'NO'");
            ...
            private static void RconQuitCommandAction(IWebSocketConnection socket)
            {
                Master.isClosing = true;
            ...
            private static void RconForceQuitCommandAction(IWebSocketConnection socket) { Environment.Exit(0); }

            private static void RconClearCommandAction(IWebSocketConnection socket)
            {
                Console.Clear();
            ...
            private static void RconShowModManagerCommandAction(IWebSocketConnection socket)
            {
                ServerClient toFind = NetworkHelper.GetConnectedClientFromUsername(CommandManager.commandParameters[0]);
                if (toFind == null) Logger.Error($"Player '{CommandManager.commandParameters[0]}' was not found");
                else
            ...
            private static void RconUpdateCommandAction(IWebSocketConnection socket) 
            {
                Logger.Warning("Are you sure you want to run the update command? You should only do so if you are told to, as this may break things.");
                Logger.Warning("Please type 'YES' or 'NO'");
